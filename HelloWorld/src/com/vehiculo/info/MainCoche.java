package com.vehiculo.info;

public class MainCoche {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		Coche audi = new Coche();
		Coche honda = new Coche (3);
		Coche opel = new Coche(105.0f,19.0f);
		Coche mercedes = new Coche(50.0f , 350.0f , 5, 8);
		//invocacion de lo smetodos estaticos
		float kw = Coche.convertirCvKw(350.0f);
		float = cv = Coche.convertirKwCv(kw);
	}	
	//Mostrar el resultado de los dos calculos de los metodos static
	System.out.println(kw);
	System.out.println(cv);
	
	//tmbien se puede llamar al metodo estatico desde un objeto
	mercedes.convertirCvKw(350.0f);
	
}
